...
from cuml.metrics.cluster.silhouette_score import cython_silhouette_samples
...
class MFEClustering:
    ...
    def precompute_clustering_class(
        ...
        **kwargs
        ...
        if y is not None and not {"classes", "class_freqs"}.issubset(kwargs):
            classes, class_freqs = cp.unique(y, return_counts=True)

            precomp_vals["classes"] = cp.asnumpy(classes)
            precomp_vals["class_freqs"] = cp.asnumpy(class_freqs)
        ...
        **kwargs
        ...
        if (
            ...
            if class_freqs is None:
                _, class_freqs = cp.unique(y, return_counts=True)

            if n_neighbors is None:
                n_neighbors = int(cp.sqrt(class_freqs.min()))
    ...
    def _calc_pwise_norm_intercls_dist(
        ...
        if cls_inds is None:
            ...
            cls_inds = _utils.calc_cls_inds(y=y, classes=cp.asnumpy(classes))
    ...
    def _calc_all_intracls_dists(
        ...
        if cls_inds is None:
            ...
            cls_inds = _utils.calc_cls_inds(y=y, classes=cp.asnumpy(classe))

        intracls_dists = cp.array(
            [
    ...
    def _get_nearest_neighbors(
        ...
        model = KDTree(N, metric=dist_metric)
    ...
    def _get_class_representatives(
        ...
        representative_arr = cp.asarray(representative)
        ...
        return cp.ndarray.get(representative_arr)
    ...
    def ft_sil(
        ...
        # )
        silhouette= cython_silhouette_samples(
            X=N,
            labels=y,
            metric=dist_metric           
        )
    ...
    def ft_pb(
        ...
        """
        inst_dists = cdist(X=N, metric=dist_metric)

        inst_matching_classes = cp.array(
            [
        ...
        correlation, _ = scipy.stats.pointbiserialr(
            x=cp.asnumpy(inst_matching_classes), y=inst_dists
        )
    ...
    def ft_nre(
        ...
        return entropy(class_freqs / num_inst)
